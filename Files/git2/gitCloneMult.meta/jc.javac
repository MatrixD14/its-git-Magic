public class gitCloneMult {
    ...
    new AsyncTask(
        new AsyncRunnable() {
          ...
          public void onEngine(Object result) {
            Terminal.log("Download finalizado");
  ...
  public void processFile() {
    try {
      ...
      Terminal.log("erro no json: " + e.getMessage());
    }
  }
    ...
    try {
      ...
      for (GitCloneJson json : file) {
        if (json.type.equals("file")) {
          ...
          Terminal.log(inforData.toString());
          Terminal.log(destino.exists()?"já existe sobrescrevel":"");
          ...
          try {
            ...
            FileJson(result.toString(),pathAll);
              ...
              Terminal.log("erro ao acessar a path: "+subPath+ " -->" + e.getMessage());
          } 
        }
      }
    } catch (Exception e) {
