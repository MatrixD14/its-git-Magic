  ...
  public void GitClone(String link, String path) {
    ...
    try {
      ...
      if (com.getResponseCode() != 200) {
          Terminal.log("erro no Http: "+ com.getResponseCode());
        return;
      } 
      ...
      if (addPasth.length() == 0) Terminal.log("falho file void or no exists");
    } catch (IOException e) {
      Terminal.log("erro no Download file: " + e.getMessage());
    } finally {
      try {
        if (fs != null) fs.close();
      } catch (IOException e) {
        Terminal.log("erro no FileOutputStream: " + e.getMessage());
      }
      try {
        if (bs != null) bs.close();
      } catch (IOException e) {
        Terminal.log("erro no BufferedInputStream: " + e.getMessage());
      }
      try {
        if (in != null) in.close();
      } catch (IOException e) {
        Terminal.log("erro no InputStream: " + e.getMessage());
      }
    }
  }
}